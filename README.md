...
- **标准标签**：可选项，仅由用户显式选择/填写；绝不按项目名自动映射、猜测或自动合并；待确认项目不参与趋势；标准标签仅作兼容保留，不影响趋势。
- **「识别数据」（黑盒流程）**：报告有图片后显示一个「📷 识别数据」按钮 → 直接在本机读取图片 → 自动把读出的文字交给配置的第三方服务整理成检查项目 → 进度只显示「正在读取图片 / 正在整理检查项目」→ 成功后显示识别出的项目供检查/编辑（全部待确认、无标准标签、不会自动进入趋势），由你点击「添加到报告」。失败只给自然语言错误与「重试」。
...
- **标准标签（可选项）**：仅由用户显式选择/填写；绝不按项目名自动映射、猜测或自动合并；待确认项目不参与趋势；标准标签仅作兼容保留，不影响趋势。用户「采用」的推荐标签保存为家庭级/本地映射（LabelMapping），可在下次识别复用。
...
- **默认全部在本机**：健康数据与附件（图片/PDF）只保存在本设备浏览器；图片仅在本机读取，**图片不会被发送**。
- **只有少数必要信息会发送**：当你点击「识别数据」后，从图片中读出的文字会通过**同源代理**（Vite dev server 的 `/api/recognize-report`）发送给你配置的第三方服务，用于整理成检查项目；随附的内容仅有：识别用途标记、受控目录简表（id+名称）与你已确认的别名映射（仅名称到 ID，**不含任何历史健康数值**）。图片、密钥、报告全文、健康数值历史一律不发送。第三方服务的数据处理政策以其条款为准（详见界面右下角「关于与隐私说明」）。
...
src/
  ...
  components/
    ReportRecognitionPanel.tsx  # 「识别数据」黑盒流程（本机读取 → 同源代理整理 → 核对/添加）
    ImageCropModal.tsx          # 图片编辑弹窗（tui-image-editor，纯本地：cropzone 裁剪 / pixelate 马赛克）
    PrivacyModal.tsx            # 隐私说明弹窗
    MemberManager / ReportManager / TrendView / DataManager / Kit / MiniLineChart
  utils/
    ocr.ts                  # 本机文字识别会话（tesseract.js，只输出文字）
    ocrEngine.ts            # Tesseract 单一引擎工厂与统一接口
    ocrPreprocess.ts        # 本机图像预处理（灰度等，纯像素函数可单测）
...
- `architectureConstraints.test.ts`：源级约束——前端源码无 `VITE_*`/`OPENCODE_GO_*`/上游地址/鉴权头（旧 `VITE_OCR_ENGINE` / `VITE_OCR_PADDLE_*` 已随 Paddle 引擎一并移除，不再有白名单豁免），代理仅存在于 vite.config.ts（Node 侧），旧技术入口已删除，本机读取为纯本地，隐私入口存在；
