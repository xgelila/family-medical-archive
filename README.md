...
- **「识别数据」（黑盒流程）**：报告有图片后显示一个「📷 识别数据」按钮 → 打开图片编辑（成熟第三方库 tui-image-editor：原生 cropzone 裁剪（可整体拖动、四角/边框调整大小）、旋转、马赛克（原生 pixelate 像素化滤镜）、撤销/重做）→ 确认后自动在本机读取图片 → 自动把读出的文字交给配置的第三方服务整理成检查项目 → 进度只显示「正在读取图片 / 正在整理检查项目」→ 成功后显示识别出的项目供检查/编辑（全部待确认、无标准标签、不会自动进入趋势），由你点击「添加到报告」。失败只给自然语言错误与「重试」。
