...
字段说明：
- report 与 items 的字段值保持原文用字（如 Al、A1 不得改成 AI），缺失一律填空字符串 ""；
- items 是检查项目列表：name=项目名、result=结果值原文（数值或定性，如 "5.6"、"阴性"；含 < > ≤ ≥ 前缀时保留）、
...
- items 是检查项目列表：name=项目名（保持原文用字，如 Al、A1 不得改成 AI）、result=结果值原文（数值或定性，含 < > ≤ ≥ 前缀时保留）、
  referenceRange=参考区间、unit=单位、method=检验方法（缺失填空）、sourceText=该项目来源的原文片段（须与输入逐字一致）；
