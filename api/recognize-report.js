...
function responseError(res, status, message, code, id, stage) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const target = endpoint(process.env.DEEPSEEK_ENDPOINT);
    const scheme = process.env.DEEPSEEK_AUTH_SCHEME === undefined ? 'Bearer' : String(process.env.DEEPSEEK_AUTH_SCHEME).trim();
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 60000);
    try {
      const upstream = await fetch(target, { method: 'POST', signal: controller.signal, headers: { 'content-type': 'application/json', accept: 'application/json', [process.env.DEEPSEEK_AUTH_HEADER || 'Authorization']: scheme ? `${scheme} ${key}` : key }, body: JSON.stringify({ model, messages: [{ role: 'system', content: `${SYSTEM_PROMPT}\\n重点：识别${body.mode === 'report' ? '整张报告' : '检查项目'}。` }, { role: 'user', content: body.text }], temperature: 0, thinking: { type: 'disabled' } }) });
      if (!upstream.ok) return responseError(res, upstream.status >= 400 && upstream.status < 600 ? upstream.status : 502, errorMessage(upstream.status), `UPSTREAM_HTTP_${upstream.status}`, id, 'upstream-request');
      ...
      return responseError(res, controller.signal.aborted ? 504 : 502, errorMessage(controller.signal.aborted ? 504 : 502), controller.signal.aborted ? 'UPSTREAM_TIMEOUT' : 'UPSTREAM_CONNECTION_ERROR', id, 'upstream-request');
